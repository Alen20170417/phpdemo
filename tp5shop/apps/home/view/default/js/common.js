...
WST.loginWindow = function(){
	var loading = WST.load({msg:'正在加载'});
	$.post(WST.U('home/users/toLoginBox'),{},function(data,textStatus){
		layer.close(loading);
	    WST.open({type:1,area:['550px','320px'],offset:['150px'],title:'用户登录',content:data});
	});
}
/********************* 选项卡切换隐藏 **********************/
...
WST.checkCart = function(){
	$('#list-carts2').html('');
	$('#list-carts3').html('');
	$('#list-carts').html('<div style="padding:32px 0px 77px 112px;"><img src="'+WST.conf.ROOT+'/apps/home/view/default/img/loading.gif">正在加载数据...</div>');
...
WST.cancelFavorite = function(obj,type,id){
	if(window.conf.IS_LOGIN==0){
		WST.loginWindow();
		return;
	}
...
WST.addFavorite = function(obj,type,id){
	if(window.conf.IS_LOGIN==0){
		WST.loginWindow();
		return;
	}
	$.post(WST.U('home/favorites/add'),{type:type,id:id},function(data,textStatus){
	     ...
	     if(json.status==1){
	    	 ...
	    	 $(obj).html('已关注')[0].onclick = function(){
	    		 WST.cancelFavorite(obj,type,id); 
	    	 };
	     }else{
	    	 WST.msg(json.msg,{icon:2});
	     }
