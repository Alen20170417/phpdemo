...
function upload(n){
    var uploader =WST.upload({
        ...
        callback:function(f,file){
          ...
          var btn = $('<div style="position:relative;top:-80px;left:60px;cursor:pointer;" ><img src="'+WST.conf.ROOT+'/apps/Home/View/default/img/seller_icon_error.png"></div>');
          tdiv.append(btn);
...
function userAppraise(p){
  $('#list').html('<img src="'+WST.conf.ROOT+'/apps/home/view/default/img/loading.gif">正在加载数据...');
  var params = {};
  params = WST.getParams('.s-query');
  params.key = $.trim($('#key').val());
  params.p = p;
