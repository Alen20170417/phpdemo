 ...
 * 用户控制器
 */
use apps\home\model\Users as MUsers;
use apps\home\model\LogSms;
class Users extends Base{
	...
	public function login(){
		...
		return view('default/user_login');
	}
	...
	public function protocol(){
		return view("default/user_protocol");
	}
	...
	public function index(){
		...
		return view('default/users/index');
	}
	...
	*/
	public function edit(){
		...
		return view('default/users/user_edit');
	}
	/**
	* 跳去修改密码页
	*/
    ...
    */
    ...
    public function security(){
    	...
    	$this->assign('data',$data);
    	return view('default/users/security/index');
    }
    /**
     * 修改邮箱页
     */
    ...
    public function getEmailVerify(){
    	...
    	$key = $base64->encrypt($userEmail."_".session('WST_USER.userId')."_".time()."_".$process,(int)session('WST_USER.loginSecret'),30*60);
    ...
    public function emailEdit(){
    	...
    	$key = $keyFactory->decrypt($key,(int)session('WST_USER.loginSecret'));
    	$key = explode('_',$key);
     ...
     * 发送验证邮件/修改邮箱
     */
    public function getEmailVerify2(){
    	$m = new MUsers();
    	...
    	$base64 = new \org\Base64();
    	$key = $base64->encrypt("0_".session('WST_USER.userId')."_".time(),(int)session('WST_USER.loginSecret'),30*60);
    	$url = url('Home/Users/emailEdit2',array('key'=>$key),true,true);
    	$html="您好，会员 ".session('WST_USER.loginName')."：<br>
    ...
    public function toApply(){
    	return view("default/user_verify_phone");
    }
    /**
     * 绑定手机/获取验证码
     */
    public function getPhoneVerify1(){
    	$userPhone = input("post.userPhone");
  ...
  //  	if($rv['status']==1){
    ...
    //	}
    	return WSTReturn($phoneVerify,1);
    }
    /**
     * 绑定手机
     */
     ...
     * 修改手机/获取验证码
     */
    public function getPhoneVerify2(){
    	$m = new MUsers();
    	$data = $m->getById(session('WST_USER.userId'));
    ...
    */
    ...
    public function checkfindPhone($phoneVerify){
    	...
    	if (session('findPhone.phoneVerify') == $phoneVerify ) {
    		...
    		if(!empty($fuserId)){
    			$rs['status'] = 1;
    			$keyFactory = new \org\Base64();
    			$key = $keyFactory->encrypt("0_".session('findPass.userId')."_".time(),(int)session('WST_USER.loginSecret'),30*60);
    			$rs['url'] = url('Home/Users/forgetPass3',array('key'=>$key),true,true);
    			return $rs;
    		}
    ...
    public function getfindEmail(){
    	$base64 = new \org\Base64();
    	$key = $base64->encrypt("0_".session('findPass.userId')."_".time(),(int)session('WST_USER.loginSecret'),30*60);
    	$url = url('Home/Users/forgetPass3',array('key'=>$key),true,true);
