...
class Carts extends Base{
	...
	/**
	 * 删除指定购物车里的指定商品
	 * *暂时写的接口，当做购物车的过程中，请继续完善
	 */
	public function delGoodsById($goodsId){
	...
	public function checkGoodsSaleSpec($goodsId,$goodsSpecId){
		...
		//有规格的话查询规格是否正确
		if($goods['isSpec']==1){
			$specs = model('GoodsSpecs')->where(['goodsId'=>$goodsId,'dataFlag'=>1])->field('id,isDefault,specStock')->select();
			if(count($specs)==0){
	...
	public function getCarts($isSettlement = false){
		...
		return ['carts'=>$carts,'goodsTotalMoney'=>$goodsTotalMoney];     
	}
