...
class Articles extends Base{
	...
	*/
	public function getHelpById(){
		$id = (int)input('id');
		return $this->alias('a')->join('__ARTICLE_CATS__ ac','a.catId=ac.catId','inner')->where('ac.parentId=7 and a.dataFlag=1')->find($id);
	}
	/**
	*  根据id获取资讯文章
	*/
	public function getNewsById(){
		$id = (int)input('id');
		return $this->alias('a')->join('__ARTICLE_CATS__ ac','a.catId=ac.catId','inner')->where('a.catId<>7 and ac.parentId<>7 and a.dataFlag=1')->find($id);
	}
	...
	*/
	...
	public function getTree(){
		$data = Db::table('__ARTICLE_CATS__')->field('catName,catId,parentId')->where('parentId <> 7 and catId <> 7 and dataFlag=1')->select();
		return $this->_getTree($data, 0);
	}
	...
	*/
	public function bcNav(){
		...
		$data = Db::table('__ARTICLE_CATS__')->field('catId,parentId,catName')->select();
		if($artId){
	...
	*/
	public function getChildIds(){
		$ids = [];
		$data = Db::table('__ARTICLE_CATS__')->select();
			foreach($data as $k=>$v){
