...
class Goods extends Base{
	...
	public function saleByPage(){
		...
		$m = new \apps\admin\model\GoodsCats();
		$keyCats = $m->listKeyAll();
	...
	public function illegal(){
		...
		try{
			...
			if($res!==false){
				...
				$message['msgContent'] = "您的商品".$rs['goodsName']."【".$rs['goodsSn']."】因【".$illegalRemarks."】被下架处理。";
				$message['createTime'] = date('Y-m-d H:i:s');
				$message['msgStatus'] = 0;
				$message['dataFlag'] = 1;
				$message['msgJson'] = json_encode(['from'=>2,'dataId'=>$id]);
	...
	public function allow(){	
		...
		try{
			...
			if($res!==false){
				...
				$message['sendUserId'] = 1;
				$message['receiveUserId'] = $rs['userId'];
				$message['msgContent'] = "您的商品".$rs['goodsName']."【".$rs['goodsSn']."】已审核通过。";
				$message['createTime'] = date('Y-m-d H:i:s');
				$message['msgStatus'] = 0;
				$message['dataFlag'] = 1;
				$message['msgJson'] = json_encode(['from'=>2,'dataId'=>$id]);
