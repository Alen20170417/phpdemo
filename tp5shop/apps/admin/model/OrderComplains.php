	 ...
	 public function getDetail(){
	 	...
	 	if($data){
			...
			$rs['log'] = Db::table('__LOG_ORDERS__')->alias('lo')
									      ...
									      ->select();
			//获取相关商品
			$rs['goodslist'] = Db::table('__ORDER_GOODS__')->where(['orderId'=>$data['orderId']])->select();
			$data['order'] = $rs;
	 	}
		return $data;
	 }
	 	...
	 	if($rs['complainStatus']==0){
		    ...
		    try{
		 	    ...
		 	    if($ers!==false){
					...
					$message['receiveUserId'] = $rs['userId'];
					$message['msgContent'] = "您有新的被投诉订单【".$rs['orderNo']."】，请及时回应以免影响您的店铺评分。";
					$message['createTime'] = date('Y-m-d H:i:s');
					$message['msgStatus'] = 0;
					$message['dataFlag'] = 1;
					$rs = model('admin/Messages')->save($message);
					Db::commit();
		    ...
		    try{
	 	        ...
	 	        if($ers!==false){
		 	    	...
		 	    	if($rs['needRespond']==1){
						...
						$message['msgType'] = 0;
						$message['sendUserId'] = 1;
						$message['receiveUserId'] = $rs['shopUserId'];
						$message['msgContent'] = "您的被投诉订单【".$rs['orderNo']."】已仲裁，请查看订单投诉详情。";
						$message['createTime'] = date('Y-m-d H:i:s');
						$message['msgStatus'] = 0;
						$message['msgJson'] = json_encode(['from'=>3,'dataId'=>$complainId]);
						$message['dataFlag'] = 1;
					...
					$message['sendUserId'] = 1;
					$message['receiveUserId'] = $rs['userId'];
					$message['msgContent'] = "您的订单投诉【".$rs['orderNo']."】已仲裁，请查看订单投诉详情。";
					$message['msgJson'] = json_encode(['from'=>3,'dataId'=>$complainId]);
					$message['createTime'] = date('Y-m-d H:i:s');
					$message['msgStatus'] = 0;
					$message['dataFlag'] = 1;
