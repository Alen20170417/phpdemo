...
class Shops extends Base{
	...
	public function pageQuery($shopStatus=1){
		return Db::table('__SHOPS__')->alias('s')->join('__AREAS__ a2','s.areaId=a2.areaId','left')
		       ->where(['s.dataFlag'=>1,'s.shopStatus'=>$shopStatus])->field('shopId,shopSn,shopName,a2.areaName,shopkeeper,telephone,shopAddress,shopCompany,shopAtive,shopStatus')->paginate(input('pagesize/d'));
	}
	/**
	 * 删除
	 */
	...
	public function getById($id){
		...
		//获取经营范围
		$goodscats = model('CatShops')->where('shopId',$id)->select();
		$shop['catshops'] = [];
		foreach ($goodscats as $v){
			$shop['catshops'][$v['catId']] = true;
		}
		//获取认证类型
	    $shopAccreds = model('ShopAccreds')->where('shopId',$id)->select();
	    $shop['accreds'] = [];
	...
	public function add(){
        ...
        try{
        	...
        	if($userId==0){
        		...
        		model('users')->where('userId',$userId)->save(['userType'=>1]);
        	}
	        $data = Input('post.');
	...
	public function edit(){
        ...
        try{
		    ...
		    //经营范围
		    CatShops::where('shopId','=',$shopId)->delete();
		    $goodsCats = explode(',',$goodsCatIds);
		    $cs = model('CatShops');
		    foreach ($goodsCats as $key =>$v){
		        if((int)$v>0){
		        	$cs->create(['shopId'=>$shopId,'catId'=>$v]);
		        }
		    }
		    //认证类型
		    ShopAccreds::where('shopId','=',$shopId)->delete();
	        if($accredIds!=''){
	            $accreds = explode(',',$accredIds);
	            $sa = model('ShopAccreds');
		        foreach ($accreds as $key =>$v){
			        if((int)$v>0){
			        	$sa->create(['shopId'=>$shopId,'accredId'=>$v]);
			        }
			    }
	        }
	        Db::commit();
